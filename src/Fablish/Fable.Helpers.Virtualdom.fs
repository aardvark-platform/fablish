...
module Html =
    ...
    module Svg = 
        ...
        let inline path x = svgElem "path" x
...
module Marshaling =
    ...
    let createObj (xs : #seq<string*obj>) =
        let fields = xs |> Seq.map (fun (k,v) -> if v.GetType() = typeof<string> then sprintf "%s : %s" k (unbox v) else sprintf "%s: %A" k v) |> String.concat "; "
        sprintf "{ %s }" fields

    let (==>) (k : string) (v : obj) = k,v
    ...
    let toAttrs (attrs:Attribute<'T> list) =
        ...
        let props =
            ...
            |> List.map (function
                ...
                | EventHandler(ev,f) -> 
                    let id = System.Guid.NewGuid().ToString()
                    ev ==> sprintf "function(ev) { send('%s'); }" id, Some (id, f)
            )
        let registrations = props |> List.choose (function (_,None) -> None | _,Some v -> Some v )
        let props = props |> List.map fst

        match elAttributes with
        | None -> props
        | Some x -> x::props
        |> createObj, registrations
    let attributes,registrations = toAttrs attributes
    sprintf "h('%s',%s,[%s])" tag attributes (String.concat "," children), registrations
...
let rec render handler node =
    ...
    | Svg((tag,attrs), nodes) -> 
        let nodes = nodes |> List.map (render handler)
        let r = nodes |> List.map snd |> List.concat
        let (dom,t) = createTree handler tag attrs (nodes |> List.map fst)
        dom,r@t
    | VoidElement (tag, attrs) -> createTree handler tag attrs []
    | Text str -> sprintf "String('%s')" str, []
    | WhiteSpace str -> sprintf "String('%s')" str, []


module VirtualDomRenderer =
    open Aardvark.Base.Monads.State

    let mkKvps (xs : list<string*string>) =
        xs |> List.map (fun (k,v) -> sprintf "'%s' : '%s'" k v) |> String.concat ","

    let createTree (handler : 'a -> unit) (tag : string) (attributes : Attribute<'a> list) (children : seq<string>) =
        state {
            let elAttribs = 
                attributes 
                    |> List.choose (function Attribute(k,v) -> Some (sprintf "'%s' : '%s'" k v) | _ -> None)
            ...
            let props = elAttribs @ properties |> String.concat ","

            return sprintf "h('%s',{%s},[%s])" tag props (children |> String.concat ",")
        }
    
    let rec render handler node : State<_,string> =
        state {
            match node with
                | Element((tag,attrs), nodes) | Svg((tag,attrs), nodes) -> 
                    let! nodes = nodes |> List.mapS (render handler)
                    return! createTree handler tag attrs nodes
                | VoidElement (tag, attrs) -> 
                    return! createTree handler tag attrs []
                | Text str -> 
                    return sprintf "String('%s')" str
                | WhiteSpace str -> 
                    return sprintf "String('%s')" str
        }


module ReactDomRenderer =
    open Aardvark.Base.Monads.State
    ...
    let createTree (handler : 'a -> unit) (tag : string) (attributes : Attribute<'a> list) (children : seq<string>) =
        state {
            ...
            let guarded = if props.Length = 0 then "null" else sprintf "{ %s }" props

            if Seq.length children = 0 then
                return sprintf "React.createElement('%s',%s)" tag guarded
            else
                return sprintf "React.createElement('%s',%s,%s)" tag guarded (children |> String.concat ",")
        }
